...
//    NSString *result = [self stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"%@&&typeof(%@)==\"function\"",jsFunctionName,jsFunctionName]];
    NSString *result = [self stringByEvaluatingJavaScriptFromString:[NSString stringWithFormat:@"%@",jsFunctionName]];
    if([result isEqualToStringCheck:@"true"]) {
        return YES;
    }
    return NO;
}
...
- (BOOL)isJSVariable:(NSString *)jsVariableName {
    ...
    if([result isEqualToStringCheck:@"false"]) {
        return YES;
    }
    return NO;
}
...
- (BOOL)dispatchURL:(NSURL *)url {
    if([url.scheme isEqualToStringCheck:kBridgeScheme]) {
...
//
//        }else{
            NSString *jsonString = [NSString stringWithFormat:@"%@", result];
            NSString *js = [NSString stringWithFormat:@"__CallbackCaches__[\"%@\"](decodeURIComponent(\"%@\"))",jsMethod,[jsonString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]];
            [self stringByEvaluatingJavaScriptFromString:js];
//        }
