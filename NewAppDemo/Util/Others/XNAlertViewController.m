...
#import "XNAlertViewController.h"

@interface XNHighLightButton : UIButton

@property (strong, nonatomic) UIColor *highlightedColor;

@end

@implementation XNHighLightButton
...
@interface XNAlertAction ()

@property (copy, nonatomic) void(^actionHandler)(XNAlertAction *action);

@end

@implementation XNAlertAction

+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(XNAlertAction *action))handler {
    XNAlertAction *instance = [XNAlertAction new];
...
@interface XNAlertViewController ()
...
@implementation XNAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    
    XNAlertViewController *instance = [XNAlertViewController new];
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        XNHighLightButton *btn = [XNHighLightButton new];
        btn.tag = 10+i;
        btn.highlightedColor = UIColorFromRGB(0x356BFE);
...
- (void)creatAllSeparatorLine {
    ...
    for (int i=0; i<linesAmount; i++) {
        ...
        separatorLine.backgroundColor = UIColorFromRGB(0xD8D8D8);
...
- (void)didClickButton:(UIButton *)sender {
    XNAlertAction *action = self.actions[sender.tag-10];
